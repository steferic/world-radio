# face_speaker.py — the speaker face: angled louvres over the driver.
#
# Same construction as the other faces (110mm cream deck, 98.2mm flush black
# plate at 6mm radius, 0.5mm hairline part-line groove). The plate is cut with
# angled stadium louvres that open into the acoustic cavity; a faint disc behind
# them hints at the driver cone, so the slots read as real openings.
#
# The louvre field is generated long, then INTERSECTED with an inset rounded
# square so the slots are clipped to a neat field with an even margin, and the
# clipped set is subtracted from the plate in one boolean.
#
#   /Applications/Blender.app/Contents/MacOS/Blender --background --python face_speaker.py
...
# ---- louvres ----
LOUVRE_ANGLE = 35.0     # degrees from horizontal
LOUVRE_W = 3.3          # slot width
LOUVRE_PITCH = 6.6      # slot + land
FIELD_MARGIN = 5.0      # black plate visible around the louvre field
CAVITY_DEPTH = 26.0     # how deep the darkness behind the slots reads
DRIVER_DIA = 76.0       # 3" driver hint
OFF_LIMIT = 55.0        # drop slots that would only graze the field corners
...
def disc(name, cx, cy, dia, z0, z1, verts=96):
    bm = bmesh.new()
    r = dia / 2
...
def louvre_cutters(name, z0, z1):
    """One mesh holding every slot: long stadium slots at LOUVRE_ANGLE."""
    a = math.radians(LOUVRE_ANGLE)
    ca, sa = math.cos(a), math.sin(a)
    span = PLATE * 1.6                      # long enough to cross the field
    n = int(span / LOUVRE_PITCH / 2) + 1
    bm = bmesh.new()
    for i in range(-n, n + 1):
        off = i * LOUVRE_PITCH
        if abs(off) > OFF_LIMIT:            # avoids corner sliver slots
            continue
        base = rrect_pts(span, LOUVRE_W, LOUVRE_W / 2)
        pts = [(x * ca - (y + off) * sa, x * sa + (y + off) * ca) for (x, y) in base]
        add_solid(bm, pts, z0, z1)
    return finish(bm, name)
...
def build():
    ...
    ink_mat = matte("Plate", (0.013, 0.014, 0.016), rough=0.42, coat=0.06)
    void_mat = matte("Cavity", (0.004, 0.004, 0.005), rough=0.85)
    cone_mat = matte("Driver", (0.030, 0.030, 0.034), rough=0.62)

    field = INLAY - 2 * FIELD_MARGIN

    # ---- cream deck, pocketed for the flush plate, opened under the field ----
    deck = prism("Deck", 0, 0, PLATE, PLATE, DECK_R, 0, PT)
    boolean(deck, prism("void", 0, 0, field + 2, field + 2, INLAY_R,
                        -30, PT - INK_T + 0.05))
    ...
    # ---- black plate, louvred ----
    plate = prism("Plate", 0, 0, INLAY - 0.1, INLAY - 0.1, INLAY_R, PT - INK_T, PT)
    cut = louvre_cutters("louvres", PT - INK_T - 1, PT + 1)
    intersect(cut, prism("region", 0, 0, field, field, INLAY_R + 2, PT - INK_T - 2, PT + 2))
    boolean(plate, cut)
    bevel(plate, 0.18, segments=2, angle_limit=45)
    smooth(plate, angle=0.6)
    assign(plate, ink_mat)

    # ---- what you see THROUGH the slots ----
    cav = prism("Cavity", 0, 0, field + 6, field + 6, INLAY_R,
                -CAVITY_DEPTH, PT - INK_T - 0.15)
    boolean(cav, prism("cav_in", 0, 0, field + 2, field + 2, INLAY_R,
                       -CAVITY_DEPTH + 3, PT + 1))
    assign(cav, void_mat)
    cone = disc("Driver", 0, 0, DRIVER_DIA, -9.5, -7.5)
    assign(cone, cone_mat)

    # ---- studio ----
    bpy.ops.mesh.primitive_plane_add(size=3.4, location=(0, 0.55, mm(-CAVITY_DEPTH - 2)))
...
if __name__ == "__main__":
    ...
    render(f"{OUT_DIR}/face_speaker_top.png")
    bpy.data.objects.remove(bpy.context.scene.camera, do_unlink=True)
    cam((mm(200), mm(-250), mm(205)), 56)
    render(f"{OUT_DIR}/face_speaker_hero.png")
