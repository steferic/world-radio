...
// Show a single centered status line (e.g. "Connecting to WiFi...").
...
// middle), artist (small, below title). Any string may be empty -- pass
// "" rather than nullptr. Clears the screen before drawing, so call it
// only when metadata actually changes rather than every loop iteration.
